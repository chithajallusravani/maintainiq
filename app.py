...
def load_models():
    ...
    return {
        ...
        "failure_type": failure_type_model,
        "encoder": encoder,
        "rul": rul_model,
        "repair_cost": repair_model,
        "paths": paths,
    }
...
def login_page():
    ...
    with center:
        ...
        st.caption(
            "Access your predictive maintenance control center."
        )
...
try:
    ...
    st.error(
        "MaintainIQ deployment models could not be loaded."
    )
...
def prepare_input(
    ...
    if missing:
        ...
        raise ValueError(
            "Saved preprocessor expects unsupported "
            f"features: {missing}"
        )
...
def decode_failure_type(raw_type):
...
def prediction(input_df):
    ...
    failure_type = decode_failure_type(
        raw_type
    )
...
def cards(result):
    ...
    for col, data_item in zip(
        cols,
        data,
    ):

        icon, label, value, note, color = data_item
...
def home():
    ...
    with a:
        ...
        st.markdown(
            "### 🚀 What MaintainIQ Does"
        )

        st.write(
            "MaintainIQ converts machine-health and "
            "operating information into practical "
        ...
        st.success(
            "All deployment artifacts are loaded."
        )
...
def dashboard():
    ...
    if not st.session_state.history:
        ...
        st.info(
            "No predictions yet. Generate a prediction first."
        )
...
def predictions():
    ...
    st.info(
        ...
        "selected deployment models → risk, type, RUL and repair-cost predictions."
    )
        ...
        try:
            ...
            if (
                ...
                st.warning(
                    ...
                    "predicted 24-hour failure probability "
                    "is at least 50%."
                )
                ...
                st.success(
                    "No failure is predicted within 24 hours "
                    "at the current model threshold."
                )
...
def performance():
    ...
    st.success(
        "Each ML task was evaluated independently. "
        "The best task-specific model was selected for deployment."
    )
    ...
    st.caption(
        "Primary metric: F1 Score. Deployment: MLP."
    )
    ...
    st.caption(
        "Primary metric: MAE. Deployment: XGBoost."
    )
    ...
    st.info(
        ...
        "detection, Macro F1 for failure-type classification, "
    ...
    st.success(
        ...
        "used by MaintainIQ predictions."
    )
...
def about():
    ...
    with a:
        ...
        st.write(
            "**MaintainIQ** is an intelligent "
            "predictive-maintenance application "
            "designed to predict machine risk before "
    ...
    with b:
        ...
        st.write(
            ...
            "can identify risk, understand likely failure "
            "mode, estimate remaining machine life and "
    ...
    st.write(
        ...
        "temperature_motor, current_phase_avg, "
    ...
    st.caption(
        "Saved preprocessing performs numeric imputation/scaling "
    ...
    st.info(
        ...
        "deployment. MaintainIQ loads the saved artifacts "
